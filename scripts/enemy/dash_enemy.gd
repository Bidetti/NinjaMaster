...
func update_timers(delta):
	...
	if dash_timer > 0:
		dash_timer -= delta
		if dash_timer <= 0 and not is_dead and player_ref:
			prepare_dash_attack()
...
func execute_dash_enemy_movement(delta):
	match dash_enemy_state:
		DashEnemyState.WALKING, DashEnemyState.RUNNING:
		...
		DashEnemyState.DASH_ATTACK:
			...
			move_and_slide()
			# Dano ao player durante o dash
			if player_ref and global_position.distance_to(player_ref.global_position) < 20.0:
